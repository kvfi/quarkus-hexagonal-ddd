package com.acme.infrastructure.rest.api;
...
import com.acme.infrastructure.rest.dto.ResponseMessageDTO;
import com.acme.infrastructure.rest.mapper.LoanDTOMapper;
...
public class LoanResource implements LoansAPI {
  ...
  public Response doGetAllLoans() {
    ...
    if (!loans.isEmpty()) {
      response =
          Response.ok(loans.stream().map(p -> loanMapper.toDto(p)).collect(Collectors.toList()))
              .build();
    } else {
  ...
  public Response doCreateLoan(LoanDTO loanDTO) {
    ...
    ResponseMessageDTO response =
        ResponseMessageDTO.builder().message("Loan created successfully").build();
  ...
  public Response doDeleteLoanById(UUID id) {
    ...
    ResponseMessageDTO response =
        ResponseMessageDTO.builder().message("Loan deleted successfully").build();
